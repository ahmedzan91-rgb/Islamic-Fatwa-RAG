...
2. **الإحالة الإلزامية**: اذكر بعد كل حكم أو معلومة رقم مصدرها بأقواس مربّعة عادية هكذا بالضبط: [مصدر 1]
   ⚠️ استخدم القوسين `[` و `]` الإنجليزيين حصراً — لا تستخدم 【】 ولا «» ولا () ولا أرقاماً هندية.
   ولا تذكر مصدراً لم يرد في السياق.
...
**المصادر المعتمدة:**
(اكتب أرقام المصادر التي استشهدت بها فقط، هكذا: [مصدر 1]، [مصدر 3])
...
def normalize_citations(text: str) -> str:
    """
    توحيد أشكال الإحالة إلى [مصدر ن].

    النماذج تكتب الإحالة بأقواس متنوّعة حسب لغة تدريبها:
      【مصدر 1】 (صينية) · «مصدر 1» · (مصدر 1) · [ مصدر ١ ]
    نوحّدها جميعاً كي يعمل مؤشر التأصيل والربط بجدول المصادر.
    """
    if not text:
        return text
    ...
    # الأقواس غير القياسية → أقواس مربّعة
    for open_ch, close_ch in [("【", "】"), ("〔", "〕"), ("《", "》"),
                              ("«", "»"), ("﴿", "﴾")]:
        text = _re.sub(
            rf"{_re.escape(open_ch)}\s*(مصدر\s*[\d٠-٩]+)\s*{_re.escape(close_ch)}",
            r"[\1]", text,
        )

    # الأقواس الهلالية: (مصدر 3) → [مصدر 3]
    text = _re.sub(r"\(\s*(مصدر\s*[\d٠-٩]+)\s*\)", r"[\1]", text)

    # تحويل الأرقام الهندية داخل الإحالات
    def _to_western(match):
        digits = match.group(2).translate(str.maketrans("٠١٢٣٤٥٦٧٨٩", "0123456789"))
        return f"[مصدر {digits}]"

    text = _re.sub(r"\[\s*(مصدر)\s*([\d٠-٩]+)\s*\]", _to_western, text)
    return text
...
def verify_groundedness(answer: str, num_sources: int) -> bool:
    ...
    """
    return bool(extract_citations(answer, num_sources))


def extract_citations(answer: str, num_sources: int) -> list:
    """
    استخراج أرقام المصادر المستشهد بها فعلياً في الإجابة (مرتّبة، بلا تكرار).
    تُستخدم لعرض المصادر المقتبسة تحت الإجابة مباشرة.
    """
    ...
    found = _re.findall(r"\[\s*مصدر\s*(\d+)\s*\]", normalized)
    valid = sorted({int(c) for c in found if 1 <= int(c) <= num_sources})
    return valid
