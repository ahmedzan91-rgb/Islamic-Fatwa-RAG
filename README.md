...
## 💾 رفع بياناتك إلى المستودع

ملف 139 ألف فتوى بصيغة CSV الخام ≈ **300 MB**، وGitHub يرفض أي ملف فوق **100 MB**.
الحل: الضغط بـ gzip (النص العربي ينضغط **5–9×**) — والنظام يقرأ `.gz` مباشرة بلا فكّ.

```bash
python prepare_data_for_github.py                     # فحص الأحجام
python prepare_data_for_github.py --compress --split  # ضغط وتقسيم آلي
git add data/*.gz .gitignore && git commit -m "data" && git push
```

| الصيغة | 139 ألف فتوى | الحكم |
|---|:-:|:-:|
| CSV خام | ~300 MB | ❌ مرفوض |
| **CSV.GZ** | **~50 MB** | ✅ يمرّ |

الأداة تفحص الأحجام، تضغط، تقسّم الضخم بحجم **تكيّفي** (تقيس نسبة الضغط الفعلية
لملفك)، وتحدّث `.gitignore` تلقائياً. مُختبَرة: 40,000 صف → 9 أجزاء → استُرجعت
40,000 صف بالضبط بلا فقد.

📖 **[الدليل الكامل: UPLOAD_DATA.md](UPLOAD_DATA.md)** — سيناريوهات، مصادقة، حلّ مشاكل.
...
├── prepare_data_for_github.py   # تجهيز البيانات للرفع (ضغط + تقسيم)
